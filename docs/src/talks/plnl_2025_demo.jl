...
# ╔═╡ 8435edea-d7ec-4ce0-a02e-42b3f5221c26
using PlutoUI

# ╔═╡ fa3c8f1c-0731-4fa5-96ba-b89dc191439d
using PlutoTeachingTools

# ╔═╡ 70acdf02-c9e5-11f0-16ee-7772b4a0d2eb
using Herb

# ╔═╡ dc86c3cd-3512-4240-bcdd-812b482cc7af
using Plots

# ╔═╡ 885556eb-efd9-47ef-ad70-6b719ff0f83d
using GraphRecipes
...
md"""
## Why the need for Herb?

Current PS implementations are
...
# ╔═╡ 3f5f8237-bca3-4379-9698-c95b1eaad7aa
inputs = [1, 2, 3];

# ╔═╡ b259b128-8850-46d2-b4e7-89d93ab8484e
spec = [IOExample(Dict(:x => i), f(i)) for i in inputs];

# ╔═╡ 6f5b4574-1e1e-4d79-ba25-4f108e64bb3f
problem = Problem("Optimizing", spec);
...
plus_rule_any_children = DomainRuleNode(g, [1], [VarNode(:a), VarNode(:b)]);
...
plus_minus = DomainRuleNode(g, [1, 2], [VarNode(:a), RuleNode(4)]);
...
div_by_one = DomainRuleNode(g, [3], [VarNode(:a), RuleNode(5)]);
...
Columns(
	(@bind clear CounterButton("Clear constraints")),
	(@bind apply CounterButton("Apply constraints")),
	widths=[20, 20, 60]
)
...
let
	clear
	apply
	md"*There are now $(length(g.constraints)) constraints applied to the grammar `g`*"
end
...
# ╔═╡ 8a36844b-dee3-4ba8-abc7-58aa767f324c
md"""
and that solution is a...
"""

# ╔═╡ be7feacf-fbea-4153-8e7e-739d3f33a97e
@bind max_enumerations Slider(0:50:5000; default=4000)

# ╔═╡ cf0d8352-e0c4-4121-8712-58708750e845
(ast, result) = synth(problem, BFSIterator(g, :Int; max_depth=3); max_enumerations);

# ╔═╡ 453af393-38cc-4c93-b512-92744af9d89d
rulenode2expr(ast, g)

# ╔═╡ 8c55d184-1930-4ec1-910a-a5a936e3694a
result

# ╔═╡ ed4c525b-a8cd-410d-b788-071a75d74982
md"""
Maximum number of enumerations: $max_enumerations
"""

# ╔═╡ e04de7cc-5d82-453d-bc6d-10c6ac6ea5b1
md"""
## Next steps *for us*

We're busy polishing up the building blocks and...

- Collecting benchmarks in [`HerbBenchmarks.jl`](https://github.com/Herb-AI/HerbBenchmarks.jl/)
  - ARC, Strings, Robots, SyGuS, ...
- Re-implementing existing synthesis techniques in [`Garden.jl`](https://github.com/Herb-AI/Garden.jl)
"""

# ╔═╡ 293014a1-a182-4fa0-bb4f-b96fd7c11531
md"""
## Next steps *for you*

- Play with this presentation, *it's just a notebook*
- See [herb-ai.github.io](https://herb-ai.github.io/) for more tutorials, docs
"""
...
- a use case for yourself
- possible collaboration

or are you interested in contributing?

Please reach out to Tilman and I, or come chat with us today!
"""
...
begin
	clear # forces this cell to react when the bound button is clicked
	clearconstraints!(g)
end

# ╔═╡ 91813d11-454f-468d-a5cc-e59a862c9f13
begin
	apply # forces this cell to react when the bound button is clicked
	addconstraint!(g, order)
	addconstraint!(g, forbid_zero)
	addconstraint!(g, forbid_one)
	addconstraint!(g, contains_input)
end

# ╔═╡ 6861c591-23b9-4f93-8a4c-5c0f2a0a15f5
...
# ╔═╡ 7c30177c-7074-4c7d-89fa-27106fed0365
import HerbSearch: evaluate

# ╔═╡ 796dec9b-a453-43bd-930b-d2daeaec1b80
herb_logo = "https://herb-ai.github.io/Herb.jl/dev/assets/logo.png"

# ╔═╡ aebd1723-2eb7-4b38-9725-292a407ac0da
htl"""
<h1><img src="$herb_logo" height="100"/></h1>

<p>A Program Synthesis Framework</p>
...
<small>and many others, including: Jaap de Jong, Bart Swinkels, Pamela Wochner, Nicolae Filat, Tudor Magurescu, Issa Hanou, <strong>Sebastijan Dumancic</strong></small>
"""

# ╔═╡ c71ffdc5-1a19-4c0b-b298-ef40176e6abe
get_input(io::IOExample) = io.in

# ╔═╡ cb18b929-5f86-418c-9656-72d70de937d0
get_output(io::IOExample) = io.out

# ╔═╡ 9d404ea0-26e3-4bd6-a714-1974f667d3fc
get_spec(p::Problem) = p.spec

# ╔═╡ cda28b87-0ec6-4398-babe-9d48eb769b93
display_io(spec) = Tables.table([[i[:x] for i in get_input.(spec)] get_output.(spec)]; header = ["Input", "Output"])

# ╔═╡ 5c32ee7e-1536-4895-8fbd-ee4834eb6022
Columns(display_io(spec), g)

# ╔═╡ 2b061ce2-75e4-4476-9167-7cd8af005174
HerbSearch.evaluate(p, rn, g::AbstractGrammar) = evaluate(p, rn, grammar2symboltable(g))
...
manifest_format = "2.0"
project_hash = "d1bba7e3adff34c7d801094065415d38efa0b505"
...
git-tree-sha1 = "ac626d6d44fc57307f880dd45f6e81524d74b36d"
...
git-tree-sha1 = "54e42d0a3d771f88c8db110f50e6d1eb5ab3f1ac"
...
git-tree-sha1 = "fb23f744b2f39f708bf3de858475e8bcf429875a"
